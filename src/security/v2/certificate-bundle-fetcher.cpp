...
CertificateBundleFetcher::CertificateBundleFetcher(Face& face)
  : CertificateFetcherFromNetwork(face)
  , m_bundleInterestLifetime(100)
{
}

void
CertificateBundleFetcher::setBundleInterestLifetime(time::seconds time)
{
                                  ...
                                  const ValidationContinuation& continueValidation)
{
  ...
  if (dataValidationState == nullptr) {
    return state->fail({ValidationError::Code::IMPLEMENTATION_ERROR,
                        "Not supported yet"});
  }
  ...
  if (bundleNameTag == nullptr) {
    ...
    if (originalDataName.empty()) {
      return CertificateFetcherFromNetwork::doFetch(certRequest, state, continueValidation);
    }
                                       ...
                                       const Name& bundleName)
{
  ...
  CertificateFetcherFromNetwork::doFetch(certRequest, state, continueValidation);
}
